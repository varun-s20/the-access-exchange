...
<section class="index" id="episodes<?php echo esc_attr( $tae_suffix ); ?>" aria-labelledby="index-h<?php echo esc_attr( $tae_suffix ); ?>">
	<div class="shell">
		...
		<div class="pager">
			...
			<?php // #episodes<suffix>, not a bare #episodes: a second archive on one page would otherwise send its pager to the first one's grid. ?>
			<a href="#episodes<?php echo esc_attr( $tae_suffix ); ?>" class="btn btn--out" data-tae-more<?php echo $tae_more ? '' : ' style="display:none"'; ?>>Earlier episodes</a>
			<?php // Interviews -> Links. Handoff §23.5 says the channel is not confirmed yet. ?>
			<a href="<?php echo esc_url( tae_option( 'tae_channel_url' ) ); ?>" rel="noopener" class="btn">Subscribe</a>
		</div>
	</div>
</section>
