	...
	return array(
		'tae_interview' => array(
			...
			'tae_youtube'      => array(
				...
				'type'  => 'youtube',
				'help'  => 'Paste the whole YouTube link if that is easier - the ID is pulled out of it and saved on its own. Nothing is requested from youtube.com until a visitor presses play.',
			),
			...
			// tae_slot_watch went with the home watch poster in Phase 7. The
			// checkbox outlived the section it filled by two phases.
			...
			'tae_next'         => array(
				'label' => 'Watch next',
				'type'  => 'post_select',
				'help'  => 'The interview to put first under "Carry on from here" at the foot of this episode. Leave it alone and the three most recent other interviews are used, which is usually right - set it when two conversations genuinely belong together.',
				'none'  => '- Whatever is most recent -',
			),
			'tae_lead'         => array(
...
/**
 * Interviews as id => label, for the post_select field.
 *
 * Drafts are included, and this matters. The list was publish-only, and the
 * save handler validates against the same list - so writing a takeaway while
 * its interview was still a draft offered an empty picker, and anything posted
 * into the field was discarded without a word. The natural order of work is to
 * draft the interview, write the takeaways off the back of it, then publish;
 * that order used to lose the link silently every time.
 *
 * Unpublished ones are labelled, because picking one has a consequence: both
 * tae_parent() and tae_related() drop anything not published at render time, so
 * the link stays invisible on the front end until the interview goes live.
 *
...
function tae_interview_choices() {
	...
	foreach ( get_posts(
		...
		if ( 'publish' !== $item->post_status ) {
			$label .= ' (' . $item->post_status . ' - hidden until published)';
		}
...
function tae_add_meta_boxes() {
	foreach ( array_keys( tae_fields() ) as $type ) {
		add_meta_box(
			'tae_details',
			'tae_interview' === $type ? 'Interview details' : 'Takeaway details',
			'tae_render_meta_box',
			$type,
			'normal',
			'high'
...
function tae_render_meta_box( $post ) {
	...
	foreach ( $fields as $key => $f ) {
		...
		switch ( $f['type'] ) {
			...
			case 'youtube':
				// A plain text box. What makes it a youtube field is the save
				// handler, which reduces whatever was pasted to the bare ID.
				printf(
...
/**
 * Reduce anything a person might paste into the video field to the bare ID.
 *
 * The field used to be plain text labelled "the ID only, not the whole URL", and
 * the owner publishes interviews without a developer in the room. Pasting the
 * address bar is the obvious thing to do, it is what the YouTube share button
 * hands you, and the failure was silent: the poster still rendered, the play
 * button still worked, and the embed loaded nothing.
 *
 * So the field accepts all of these and stores dQw4w9WgXcQ:
 *
 *   dQw4w9WgXcQ
 ...
 *
 * Anything with no ID in it saves as empty rather than as a broken value, which
 * is the state the templates already handle.
 *
 * @param string $raw Whatever was typed or pasted.
 * @return string An 11-character video ID, or ''.
 */
function tae_youtube_id( $raw ) {
	...
	// Already just an ID. YouTube IDs are 11 chars of [A-Za-z0-9_-].
	if ( preg_match( '~^[A-Za-z0-9_-]{11}$~', $raw ) ) {
		return $raw;
	}

	// v= first: a watch URL can also contain /embed/ in a redirect parameter, and
	// the query string is the authoritative half of that URL.
	$patterns = array(
		'~[?&]v=([A-Za-z0-9_-]{11})~',
