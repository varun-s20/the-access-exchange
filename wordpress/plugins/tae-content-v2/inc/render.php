...
function tae_vid( $post, $classes, $size, $eager = false ) {
	// Every other helper in this file takes int|WP_Post; this one read ->ID
	// directly, so it was the one that would fatal on an ID. All four callers
	// happen to pass objects - normalising here means the fifth one cannot break it.
	$id       = is_object( $post ) ? $post->ID : (int) $post;
	$title    = get_the_title( $id );
	$video_id = (string) get_post_meta( $id, 'tae_youtube', true );
	$duration = (string) get_post_meta( $id, 'tae_duration', true );
