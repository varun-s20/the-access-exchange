/* ═══════════════════════════════════════════════════════════════════════════
   ...
   here needs jQuery.
     ...
     10  Contact  — segmented control, morphing card
...
(function () {
  ...
  if (window.TAE && window.TAE.loaded) {
    console.warn('[TAE] global.js loaded twice — the second copy is being ignored. ' +
      'Remove one of them: every click handler would otherwise fire twice and ' +
      'the menu would open then instantly close.');
    return;
  }
  var TAE = window.TAE = { version: '1.1', loaded: true, ready: false, errors: [] };
  ...
  var SITE_INDEX = window.TAE_INDEX || [
    { t: 'Home',                       d: 'The platform and its two vehicles',                       u: '/' },
    { t: 'The Interview Series',       d: 'Format, archive and how to be a guest',                   u: '/interview-series/' },
    { t: 'University Partnerships',    d: 'How a partnership is scoped, what it costs',              u: '/university-partnerships/' },
    { t: 'Insights',                   d: 'Everything we have published',                            u: '/insights/' },
    { t: 'About',                      d: 'Why this exists and what we refuse to do',                u: '/about/' },
    { t: 'Contact',                    d: 'Guests, universities, press or anything else',            u: '/contact/' },
    { t: 'Be a guest',                 d: 'Guest inquiry form',                                      u: '/interview-series/#be-a-guest' },
    { t: 'Partner with us',            d: 'University inquiry form',                                 u: '/university-partnerships/#enquire' },
    { t: 'The archive',                d: 'Every episode in full',                                   u: '/interview-series/#episodes' },
    { t: 'A term, mapped',             d: 'What a partnership looks like across twelve weeks',       u: '/university-partnerships/#s4' },
    { t: 'Cost and commitment',        d: 'What a partnership costs, lead times, procurement',       u: '/university-partnerships/#s7' },
    { t: 'From bootcamp to staff engineer in five years',                    d: 'Amara Osei, Staff Engineer',              u: '/interview-series/' },
    { t: 'What a backend interview loop actually tests',                     d: 'Ravi Menon, Engineering Manager',         u: '/interview-series/' },
    { t: 'Moving from support into product without a technical degree',      d: 'Fiona Bright, Senior Product Manager',    u: '/interview-series/' },
    { t: 'The parts of a data science job nobody puts in the job spec',      d: 'Dr Leila Haddad, Lead Data Scientist',    u: '/interview-series/' },
    { t: 'Forty applications, two offers',                                   d: 'Tobi Adeyinka, Junior Developer',         u: '/interview-series/' },
    { t: 'Building a portfolio when you have never shipped anything',        d: 'Marta Kowalska, Product Designer',        u: '/interview-series/' },
    { t: 'Switching from civil engineering at thirty-one',                   d: 'Callum Reid, Platform Engineer',          u: '/interview-series/' },
    { t: 'Internships, placements and the routes people forget exist',       d: 'Grace Nwosu, Early Careers Lead',         u: '/interview-series/' }
  ];
  ...
  function boot() {
    ...
    TAE.openMenu  = function () { setMenu(true); };
    TAE.closeMenu = function () { setMenu(false); };
    ...
    TAE.diagnose = function () {
      function name(el) {
        ...
        return (el.tagName.toLowerCase() + (c ? '.' + String(c).trim().replace(/\s+/g, '.') : ''))
               .slice(0, 90);
      }

      var v = document.getElementById('veil'), b = document.getElementById('menuBtn');
      if (!v || !b) return { fatal: 'missing ' + (!v ? '#veil' : '') + (!b ? ' #menuBtn' : '') };
      ...
      var overBtn = document.elementFromPoint(bb.left + bb.width / 2, bb.top + bb.height / 2);

      var out = {
        veilParent:  v.parentNode === document.body ? '<body>' : name(v.parentElement),
        hiddenAttr:  v.hidden,
        hasOpenClass: v.classList.contains('open'),
        veil:  { display: cs.display, visibility: cs.visibility, opacity: cs.opacity,
                 zIndex: cs.zIndex, position: cs.position },
        panel: ps && { transform: ps.transform, width: ps.width, visibility: ps.visibility,
                       background: ps.backgroundColor },
        panelBox: r && { x: Math.round(r.left), y: Math.round(r.top),
                         w: Math.round(r.width), h: Math.round(r.height) },
        onTopOfMenuButton: name(overBtn),
        clickReachesButton: !!(overBtn && (overBtn === b || b.contains(overBtn)))
      };

      if (r && r.width > 0 && r.left < innerWidth && r.top < innerHeight) {
        out.onTopOfPanel = name(document.elementFromPoint(
    ...
    console.log('[TAE] global.js ready —',
  ...
  function masthead() {
    ...
    if (!mast) return;
    var onScroll = function () { mast.classList.toggle('compact', window.scrollY > 70); };
  ...
  function menu() {
    ...
    if (trapped) console.info('[TAE] menu panel was trapped inside "' + trapped +
    ...
    setMenu = function (open) {
      ...
      if (open) {
        ...
        setTimeout(function () { veil.hidden = true; }, reduce ? 0 : 400);
      }
    };
    ...
    veil.addEventListener('click', function (e) {
      if (e.target.closest('a') || !e.target.closest('.veil-panel')) setMenu(false);
    });

    /* the header is one shared template, so mark the current page here rather
       than hard-coding aria-current into six copies of the markup */
    var here = location.pathname.replace(/\/+$/, '') || '/';
    $$('.veil-nav a').forEach(function (a) {
  ...
  function siteSearch() {
    ...
    function render(list) {
      ...
      findout.innerHTML = list.map(function (r) {
        return '<a href="' + esc(r.u) + '"><b>' + esc(r.t) + '</b><span>' + esc(r.d) + '</span></a>';
      }).join('');
    }
    ...
    function runFind() {
      ...
      render(SITE_INDEX.filter(function (r) {
    ...
    function setFind(open) {
      if (open) {
        find.hidden = false;
        requestAnimationFrame(function () { find.classList.add('open'); findq.focus(); });
        runFind();
      } else {
        find.classList.remove('open');
        findq.value = '';
        setTimeout(function () { find.hidden = true; }, reduce ? 0 : 300);
      }
      document.body.style.overflow = open ? 'hidden' : '';
    }

    if (trigger) trigger.addEventListener('click', function () { setMenu(false); setFind(true); });
    ...
    window.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && !find.hidden) { setFind(false); return; }

      if ((e.key === '/' || (e.key === 'k' && (e.metaKey || e.ctrlKey))) && find.hidden
          && !/^(INPUT|TEXTAREA)$/.test(document.activeElement.tagName)) {
        e.preventDefault(); setFind(true); return;
      }
  ...
  function lazyVideo() {
    $$('[data-yt]').forEach(function (v) {
      v.addEventListener('click', function () {
        ...
        f.src = 'https://www.youtube-nocookie.com/embed/' + v.dataset.yt +
  ...
  function prototypeForms() {
    $$('form[data-validate]').forEach(function (f) {
      f.addEventListener('submit', function (ev) {
        ...
        var bad = $$('[required]', f).filter(function (el) { return !el.checkValidity(); })[0];
        if (bad) { bad.focus(); if (bad.reportValidity) bad.reportValidity(); return; }

        var title = f.dataset.thanksTitle || 'Thank you.';
        var body  = f.dataset.thanks ||
          'That has reached us. We read every message ourselves and will come back within two working days.';
        var html  = '<div class="done"><b>' + title + '</b><p>' + body + '</p></div>';
  ...
  function homeRail() {
    ...
    if (prev) prev.addEventListener('click', function () { rail.scrollBy({ left: -step() }); });
    if (next) next.addEventListener('click', function () { rail.scrollBy({ left:  step() }); });
  ...
  function insightsIndex() {
    ...
    cards.forEach(function (c) { c._hay = c.textContent.toLowerCase().replace(/\s+/g, ' '); });
    ...
    function apply() {
      ...
      if (word)  word.textContent = shown === 1
        ? 'piece found'
        : (topic === 'all' && !term ? 'pieces published' : 'pieces found');
    ...
    if (input) {
      ...
      input.addEventListener('input', function () {
        clearTimeout(t);
        t = setTimeout(function () { term = input.value.trim().toLowerCase(); apply(); }, 110);
      });
      input.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && input.value) { input.value = ''; term = ''; apply(); }
      });
    }
    if (clear && input) {
      clear.addEventListener('click', function () {
        input.value = ''; term = ''; apply(); input.focus();
      });
    }
  ...
  /* ═══ 10 · CONTACT — segmented control, morphing card ══════════════════
     The well is measured before and after the panel swap and animated
     between the two heights, so the card grows or shrinks into the new form
     instead of jumping.
     ═══════════════════════════════════════════════════════════════════════ */
  function contactDesk() {
    ...
    var panels = $$('.panel', well);
    var release = null;

    function show(role) {
      var next = panels.filter(function (p) { return p.dataset.role === role; })[0];
      var curr = panels.filter(function (p) { return p.classList.contains('on'); })[0];
      if (!next || next === curr) return;

      var from = well.offsetHeight;
      if (curr) curr.classList.remove('on');
      next.classList.add('on');

      /* measure the new panel unpinned — a leftover inline height from an
         earlier morph would otherwise report the old size */
      well.style.height = 'auto';
      var to = well.offsetHeight;

      if (reduce || from === to) { well.style.height = ''; return; }

      next.classList.remove('enter'); void next.offsetWidth; next.classList.add('enter');
      well.style.height = from + 'px';
      requestAnimationFrame(function () { well.style.height = to + 'px'; });

      /* release on a timer, not on transitionend: an interrupted or
         zero-length transition would otherwise leave the card pinned and
         unable to grow when the success message replaces the form */
      clearTimeout(release);
      release = setTimeout(function () { well.style.height = ''; }, 620);
    }
    ...
    /* deep links: /contact/#uni opens straight on the right route */
    var hash = (location.hash || '').replace('#', '');
    if (hash && panels.some(function (p) { return p.dataset.role === hash; })) {
      var jump = seg.querySelector('[data-role="' + hash + '"]');
      if (jump) jump.click();
    }
  }
  ...
  function partnershipsGuide() {
    ...
    if (track && 'IntersectionObserver' in window) {
      var barIO = new IntersectionObserver(function (es) {
        ...
        $$('.bar', track).forEach(function (b) { b.classList.add('in'); });
        barIO.disconnect();
    ...
    if (panel) {
      var links = {};
      $$('a', panel).forEach(function (a) { links[a.dataset.to] = a; });

      if ('IntersectionObserver' in window) {
        var hereIO = new IntersectionObserver(function (es) {
        ...
        $$('.sec[id]').forEach(function (sec) { hereIO.observe(sec); });
      }
      ...
      if (stop && rail) {
        ...
        var railCheck = function () { rail.classList.toggle('gone', window.scrollY >= formTop); };
        window.addEventListener('scroll', railCheck, { passive: true });
        window.addEventListener('resize', function () {
          formTop = stop.offsetTop; railCheck();
        }, { passive: true });
        railCheck();
      }
    }
