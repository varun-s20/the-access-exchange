...
- CSS → **Elementor → Site Settings → Custom CSS**, paste `global.css`.
  Put it _there_, not in a Custom Code snippet: Elementor appends Site Settings
  Custom CSS to the end of the kit stylesheet, so it loads after Elementor's
  own Theme Style. A `<head>` snippet loads _before_ the theme and loses ties.
...
reset only _ties_ with them - and ties are broken by load order, which you do
...
The block sits deliberately _before_ the chrome and the page blocks, so every
...
| ------------------------------ | ----------------------- | -------------------------------- |
| `index.html`                   | Home                    | _(set as the static front page)_ |
...
```html
<div class="tae tae-home">
  <!-- or tae-interviews, tae-partnerships,
...
3. **Disable Elementor's default fonts/colours** (Site Settings → Layout →
   Disable Default Colors _and_ Default Fonts) so its globals do not fight
   `global.css`.
...
1. **The JS was pasted without `<script>` tags.** Elementor Custom Code and
   HTML widgets output what you give them _verbatim_. Raw JavaScript with no
...
2. **The Custom Code snippet is not live.** Elementor Custom Code has its own
   publish flow: it must be **Published** (not Draft) _and_ have Display
...
4. **A cache is serving a copy from before you added it** → purge the plugin
   cache _and_ Cloudflare/host cache, then retest in a fresh window.
...
- **The panel slides in** → the JavaScript and the CSS are both fine. The
  _click_ is not reaching the button: something is sitting on top of it.
...
Purge every cache layer after any change, and retest in a _fresh_ incognito
...
2. **The Interview Series category filter works now.** It never did. `global.css`
   §949 writes `.sorts:has(…) ~ .eps li`, but `.sorts` is _inside_ `.index-head`
